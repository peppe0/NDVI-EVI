                   ...
                   std::vector<std::thread>& background_threads)
{
    ...
    float *h_result_ndvi    = (float*)malloc(float_bytes);
    float *h_result_evi     = (float*)malloc(float_bytes);
    float *h_ndvi_mean      = (float*)malloc(float_bytes);
    float *h_ndvi_var       = (float*)malloc(float_bytes);
    float *h_ndvi_grad      = (float*)malloc(float_bytes);
    float *h_quality_score  = (float*)malloc(float_bytes);
    unsigned int  *h_quality_flags = (unsigned int*)malloc((size_t)num_pixels * sizeof(unsigned int));
    unsigned char *h_health_class  = (unsigned char*)malloc((size_t)num_pixels * sizeof(unsigned char));

    if (!h_result_ndvi || !h_result_evi || !h_ndvi_mean || !h_ndvi_var ||
        !h_ndvi_grad || !h_quality_score || !h_quality_flags || !h_health_class) {
        std::cerr << "FATAL: Host malloc failed for result buffers." << std::endl;
        free(h_result_ndvi); free(h_result_evi);
        free(h_ndvi_mean); free(h_ndvi_var); free(h_ndvi_grad);
        free(h_quality_score); free(h_quality_flags); free(h_health_class);
        cudaFree(d_red); cudaFree(d_nir); cudaFree(d_blue);
    ...
    CHECK_CUDA(cudaMemcpy(h_result_ndvi,   d_result_ndvi,   float_bytes, cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_result_evi,    d_result_evi,    float_bytes, cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_ndvi_mean,     d_ndvi_mean,     float_bytes, cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_ndvi_var,      d_ndvi_var,      float_bytes, cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_ndvi_grad,     d_ndvi_grad,     float_bytes, cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_quality_score, d_quality_score, float_bytes, cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_quality_flags, d_quality_flags, (size_t)num_pixels * sizeof(unsigned int),  cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_health_class,  d_health_class,  (size_t)num_pixels * sizeof(unsigned char), cudaMemcpyDeviceToHost));
